/// A ``ClockSourceField`` for a 3-bit clock select field which provides an external clock source.
public enum ClockSelect3BitSwapped: ClockSelectField {
  public enum TickFrequency: UInt8, ClockSelectTickFrequency {
    ...
    /// The timer will tick every 16 cycles of the internal clock.
    case every16Cycles = 0x06

    /// The timer will tick every 32 cycles of the internal clock.
    case every32Cycles = 0x07
