/// A 10-bit extension of ``PWMPin`` for ``Timer16BitPeripheral``s.
public protocol TimerPWM10BitPin: PWMPin, HasPWM10BitPin where Value == UInt16
{
  /// The ``Timer16BitChannel`` type that this pin is linked to.
  associatedtype Channel: Timer16BitChannel & TimerChannelWithPin
    where Channel.Timer: TimerFastPWM10BitConfigurable,
...
extension TimerPWM10BitPin {
  ...
  public static func configure(clock: Channel.Timer.ClockSelect, initialValue: UInt16) {
    ...
    Timer.configureModeFastPWM10Bit(clock: clock)
  }
