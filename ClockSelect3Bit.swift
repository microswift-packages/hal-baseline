/// A ``ClockSelectField`` for a 3-bit clock select field which provides an internal clock source.
public enum ClockSelect3Bit: ClockSelectField {
  ...
  public enum TickFrequency: UInt8, ClockSelectTickFrequency {
    ...
    /// The timer will tick every 32 cycles of the internal clock.
    case every32Cycles = 0x03

    /// The timer will tick every 64 cycles of the internal clock.
    case every64Cycles = 0x04

    /// The timer will tick every 128 cycles of the internal clock.
    case every128Cycles = 0x05

    /// The timer will tick every 256 cycles of the internal clock.
    case every256Cycles = 0x06

    /// The timer will tick every 1024 cycles of the internal clock.
    case every1024Cycles = 0x07
  }
  ...
  public init(bitfieldValue: UInt8) {
    ...
    if let tickFrequency = TickFrequency(rawValue: bitfieldValue) {
      self = .internal(tick: tickFrequency)
    } else {
      self = .stopped
    }
  }
